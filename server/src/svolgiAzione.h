...
void svolgiAzione(Con2DB db1, UtenteCompratore compratore, UtenteTrasportatore trasportatore, UtenteFornitore fornitore, int index_azioneDaSvolgere){

    std::cout << "UTENTE COMPRATORE DI NOME: " << compratore.nome_utente << std::endl;
    std::cout << "UTENTE FORNITORE DI NOME: " << fornitore.nome_utente << std::endl;
    std::cout << "UTENTE TRASPORTATORE DI NOME: " << trasportatore.nome_utente << std::endl;
    ...
    if (azione == "COMPRAVENDITA"){
        Product prodotto;
        Ordine ordine = prodotto.acquistaProdotto(db1, compratore.nome_utente, arr_vie_spedizione[i5], arr_citta_spedizione[i5], numeri_civici[i100]);
    }


    if (azione == "RICERCA"){
        Product prodotto;
        prodotto.ricerca_mostra_Prodotto(db1, arr_prodotti[i100]);
    }


    if (azione == "COMPRATORE_UPD_RESIDENZA"){
        compratore.aggiornaResidenza(db1, vie_di_residenza[i5], numeri_civici[i100], CAP[i5], citta_di_residenza[i5]);
    }

    
    if (azione == "COMPR_UPD_NUMTEL"){
        compratore.aggiornaNumeroDiTelefono(db1, numeri_di_telefono[i100]);
    }


    if (azione == "COMPR_UPD_PASSWORD"){
        compratore.aggiornaPassword(db1, compratore.password, password[i100]);
    }

    if (azione == "COMPR_LOGOUT"){
        compratore.effettua_logout(db1, compratore.nome_utente);
    }

    if (azione == "COMPR_LOGIN"){
        compratore.effettua_login(db1, nomi_utente[i100], password[i100]);
    }
    ...
    if (azione == "COMPR_CONTROLLA_ORDINI_EFFETTUATI"){
        Ordine ordine;
        ordine.visione_ordini_effettuati(db1,compratore.nome_utente);
    }

    // Sistema parametro
    if (azione == "INSERT_CARRELLO"){
        Carrello carrello;
        carrello.add_prodotto(db1,compratore.nome_utente, rand()%100); // da sistemare, inserire come secondo parametro il nome del prodotto.
    }

    // Sistema parametro
    if (azione == "REMOVE_CARRELLO"){
        Carrello carrello;
        carrello.remove_prodotto(db1,compratore.nome_utente, rand()%100);
    }

    // Sistema parametro
    if (azione == "INSERT_LISTA_DESIDERI"){
        ListaDesideri listaDesideri;
        listaDesideri.add_prodotto(db1,compratore.nome_utente, rand()%100); // da sistemare, inserire come secondo parametro il nome del prodotto.
    }
    ...
    if (azione == "REMOVE_LISTA_DESIDERI"){
        ListaDesideri listaDesideri;
        listaDesideri.remove_prodotto(db1,compratore.nome_utente, rand()%100);
    }


    if (azione == "AGGIUNTA_CARTA_PAGAMENTO"){
        Carta carta;
        carta.aggiungi_carta(db1,compratore.nome_utente, carte_pagamento[i100], cvv_cartePagamento[i100]);
    }

    // Sistema parametro
    if (azione == "EFFETTUA_RECENSIONE"){
        Recensione recensione;
        recensione.effettuaRecensione(db1,rand()%100, recensioni[i5], votoStelle[i5]);
    }

    // Sistema parametro
    if (azione == "RIMUOVE_RECENSIONE"){
        Recensione recensione;
        recensione.remove_recensione(db1,rand()%100);
    }

    // Sistema parametro
    if (azione == "EFFETTUA_RESO"){
        Reso reso;
        reso.effettuaReso(db1,rand()%100, motivazioniReso[i4]);
    }
    ...
    if (azione == "ANNULLA_ORDINE"){
        Ordine ordine;
        ordine.annulla_ordine(db1,rand()%100);
    }


    if (azione == "FORN_UPD_PASSWORD"){
        fornitore.aggiornaPassword(db1, fornitore.password, password[i100]);
    }

    if (azione == "FORN_LOGOUT"){
        fornitore.effettua_logout(db1, fornitore.nome_utente);
    }

    if (azione == "FORN_LOGIN"){
        fornitore.effettua_login(db1, nomi_utente[i100], password[i100]);
    }
    ...
    if (azione == "FORN_UPD_NUMTEL"){
        fornitore.aggiornaNumeroDiTelefono(db1, numeri_di_telefono[i100]);
    }
    ...
    if (azione == "FORN_REMOVE_PRODOTTO"){
        Product prodotto;
        prodotto.remove_prodotto(db1,rand()%100);
    }

    if (azione == "FORN_UPD_AZIENDA"){
        fornitore.aggiornaNomeAziendaProduttrice(db1, nomi_aziende_produttrici[i5]);
    }



    if (azione == "TRASP_UPD_PASSWORD"){
        trasportatore.aggiornaPassword(db1, trasportatore.password, password[i100]);
    }

    if (azione == "TRASP_LOGOUT"){
        trasportatore.effettua_logout(db1, trasportatore.nome_utente);
    }

    if (azione == "TRASP_LOGIN"){
        trasportatore.effettua_login(db1, nomi_utente[i100], password[i100]);
    }
    ...
    if (azione == "TRASP_UPD_NUMTEL"){
        trasportatore.aggiornaNumeroDiTelefono(db1, numeri_di_telefono[i100]);
    }

    if (azione == "TRASP_UPD_DITTA"){
        trasportatore.aggiornaNomeDittaSpedizione(db1, nomi_ditte_spedizioni[i5]);
    }
    ...
    if (azione == "TRASP_AVVISA_SPEDITO"){
        Spedizione spedizione;
        spedizione.spedizioneConsegnata(db1,rand()%100);
    }
